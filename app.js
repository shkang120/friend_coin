...
window.openAddEventModal = function(dateStr) { 
    ...
    if(!modal) { 
        modal = document.createElement('div'); modal.id = 'event-modal'; 
...
function renderHome() {
    ...
    if (!currentRoomCode) { 
        let html = `
            <div style="display:flex; gap:10px; margin-bottom:20px;">
                <button onclick="createNewRoom()" style="flex:1; padding:15px; background:#333d4b; color:white; border-radius:12px; font-weight:bold; border:none; cursor:pointer; box-shadow:0 4px 6px rgba(0,0,0,0.1); display:flex; align-items:center; justify-content:center; gap:6px;">➕ 새 클럽 개설</button>
                <button onclick="joinExistingRoom()" style="flex:1; padding:15px; background:#e8f5e9; color:#2e7d32; border-radius:12px; font-weight:bold; border:none; cursor:pointer; box-shadow:0 4px 6px rgba(0,0,0,0.05); display:flex; align-items:center; justify-content:center; gap:6px;">🔑 코드로 입장</button>
            </div>
        ...
        let html = `
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; background:#f2f4f6; padding:15px; border-radius:16px;">
                ...
                <div style="text-align:right;">
                    <div style="font-weight:bold; color:#333d4b; font-size:16px; display:flex; align-items:center; justify-content:flex-end; gap:5px;">
                        ...
                        <button onclick="changeRoomAlias('${room.room_code}', '${escapeHtml(room.room_name)}')" style="background:none; border:none; cursor:pointer; font-size:15px; padding:0; outline:none;">✏️</button>
                    </div>
            ...
            <div style="background:#f9fafb; border-radius:16px; padding:15px; margin-bottom:20px; border:1px solid #eee;">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                    <div style="font-size:14px; font-weight:bold; color:#333d4b;">💬 클럽 라운지</div>
        ...
        html += room.members.map(f => { 
            ...
            const delistedLabel = `<span style="color:#ff3b30; font-size:12px; font-weight:bold; margin-left:4px;">💀 상장폐지</span>`;
        ...
        html += `<button onclick="leaveCurrentRoom()" style="width:100%; margin-top:20px; padding:12px; background:white; color:#ff3b30; border:1px solid #ffdbdb; border-radius:12px; font-weight:bold; cursor:pointer; font-size:14px;">🚪 이 클럽에서 나가기</button>`;
        list.innerHTML = html; 
...
function renderMeeting() {
    ...
    contentHtml += displayAgendas.map(a => {
        let titleColor = '#ff3b30'; let titleText = '상장폐지 심사 법정'; let titleIcon = `🚨`;
        if (a.type === 'revival') { titleColor = '#2e7d32'; titleText = '코인 회생 재상장 건'; titleIcon = `🌱`; }
        else if (a.type === 'defense') { titleColor = '#f39c12'; titleText = '악평 이의제기 방어 법정'; titleIcon = `⚖️`; }
        else if (a.type === 'kick') { titleColor = '#8e44ad'; titleText = '클럽 내보내기 투표'; titleIcon = `🚪`; }
        ...
        const gaugeHtml = `
            <div style="position:relative; width:100%; height:14px; background:#e5e8eb; border-radius:7px; margin:15px 0; overflow:hidden;">
                <div style="position:absolute; left:0; top:0; height:100%; width:${agreePct}%; background:${titleColor};"></div>
                <div style="position:absolute; right:0; top:0; height:100%; width:${disagreePct}%; background:#3182f6;"></div>
        ...
        if (a.status === 'active') {
            ...
            btnHtml = hasVoted 
                ...
                : `<button style="flex:1; background:#f2f4f6; color:#3182f6; border:1px solid #d6ebff; font-weight:bold; padding:12px; border-radius:10px; cursor:pointer;" onclick="submitVote('${a.room_code}', '${a.id}', 'disagree')">반대 (기각)</button>
                   <button style="flex:1; background:${titleColor}; color:white; border:none; font-weight:bold; padding:12px; border-radius:10px; cursor:pointer;" onclick="submitVote('${a.room_code}', '${a.id}', 'agree')">찬성 (가결)</button>`;
        } else {
            ...
            btnHtml = `<div style="width:100%; text-align:center; padding:10px; font-size:12px; font-weight:bold; color:#8b95a1; background:#f9fafb; border-radius:10px;">종료된 재판 기록입니다.</div>`;
        }
        ...
        return `
            <div class="info-card" style="border-left: 5px solid ${titleColor}; position:relative; overflow:hidden; ${opacityStyle}" onclick="enterRoom('${a.room_code}'); switchTab('home');">
                ${stampHtml}
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
...
function renderProfile() {
    ...
    const todayStr = getFormattedDate(new Date()); const hasDailyDone = myProfile.lastDailyAttendance === todayStr; 
    
    const dailyBtn = `<button style="width: 100%; padding: 12px; border-radius: 12px; background: ${hasDailyDone ? '#e5e8eb' : '#e8f5e9'}; color: ${hasDailyDone ? '#8b95a1' : '#2e7d32'}; font-weight: bold; border: none; cursor: pointer; margin-bottom: 10px; font-size:14px;" onclick="doDailyAttendance()" ${hasDailyDone ? 'disabled' : ''}>📅 ${hasDailyDone ? '출석 완료' : '매일 출석 (+50p)'}</button>`;
    
    const hasAdBonusDone = myProfile.lastDailyAdBonus === todayStr; 
    let adDoubleBtn = ''; 
    if (hasDailyDone && !hasAdBonusDone) { 
        adDoubleBtn = `<button style="width: 100%; padding: 12px; border-radius: 12px; background: #e3f2fd; color: #1565c0; font-weight: bold; border: none; cursor: pointer; margin-bottom: 10px; font-size:14px;" onclick="watchAd('double_attendance')">🎬 광고 보고 2배 출석 (+50p)</button>`; 
    } else if (hasDailyDone && hasAdBonusDone) { 
        adDoubleBtn = `<button style="width: 100%; padding: 12px; border-radius: 12px; background: #e5e8eb; color: #8b95a1; font-weight: bold; border: none; font-size:14px;" disabled>✅ 출석 보상 2배 완료</button>`; 
    }
    
    const hasWeeklyDone = myProfile.weeklyTicketsClaimed === true; 
    const weeklyBtn = `<button style="width: 100%; padding: 12px; border-radius: 12px; background: ${hasWeeklyDone ? '#e5e8eb' : '#fff3e0'}; color: ${hasWeeklyDone ? '#8b95a1' : '#e65100'}; font-weight: bold; border: none; cursor: pointer; margin-bottom: 10px; font-size:14px;" onclick="claimWeeklyTickets()" ${hasWeeklyDone ? 'disabled' : ''}>🎁 ${hasWeeklyDone ? '주간 보너스 완료' : '주간 보너스 평가권 (각 +1장)'}</button>`;
    ...
    const mondayStr = getFormattedDate(monday); const hasAdTicketDone = myProfile.lastAdTicketMonday === mondayStr;
    
    const adTicketBtn = `<button style="width: 100%; padding: 12px; border-radius: 12px; background: ${hasAdTicketDone ? '#e5e8eb' : '#f3e5f5'}; color: ${hasAdTicketDone ? '#8b95a1' : '#6a1b9a'}; font-weight: bold; border: none; cursor: pointer; margin-bottom: 10px; font-size:14px;" onclick="watchAd('extra_ticket')" ${hasAdTicketDone ? 'disabled' : ''}>🎬 ${hasAdTicketDone ? '이번 주 추가 획득 완료' : '광고 보고 평가권 추가 (주 1회)'}</button>`;
    const logoutBtn = `<button style="width: 100%; padding: 12px; border-radius: 12px; background: #ffebee; color: #c62828; font-weight: bold; border: none; cursor: pointer; margin-top: 20px; font-size:14px;" onclick="handleLogout()">🚪 로그아웃</button>`;

    let actionBtn = `${dailyBtn}${adDoubleBtn}${weeklyBtn}${adTicketBtn}`;
    if (isDelisted) { actionBtn = `<div style="background:#ffebee; color:#c62828; padding:15px; border-radius:12px; font-weight:bold; text-align:center; font-size:14px; margin-bottom:15px;">💀 코인이 상장폐지 상태입니다.</div>`; }
    ...
    const neonBtn = neonOwned ? `<button style="background:#e5e8eb; color:#8b95a1; border:none; padding:8px 12px; border-radius:8px; font-weight:bold; font-size:12px;" disabled>✅ 보유 중</button>` : `<button onclick="buyCashItem('theme_neon')" style="background:#d4af37; color:#1c1c1e; border:none; padding:8px 12px; border-radius:8px; font-weight:bold; font-size:12px; cursor:pointer;">₩3,500</button>`;
    const fireBtn = fireOwned ? `<button style="background:#e5e8eb; color:#8b95a1; border:none; padding:8px 12px; border-radius:8px; font-weight:bold; font-size:12px;" disabled>✅ 보유 중</button>` : `<button onclick="buyCashItem('theme_fire')" style="background:#d4af37; color:#1c1c1e; border:none; padding:8px 12px; border-radius:8px; font-weight:bold; font-size:12px; cursor:pointer;">₩3,500</button>`;

    const cashShopHtml = `
        <div style="background: linear-gradient(135deg, #1c1c1e, #333d4b); border-radius:16px; padding:15px; margin-bottom:20px; color:white;">
    ...
    container.innerHTML = `
        <div style="position: relative; display: inline-block;">
            ...
            <button onclick="openProfileModal()" style="position: absolute; bottom: 0; right: -10px; background: #3182f6; color: white; border: none; border-radius: 50%; width: 32px; height: 32px; font-size: 14px; cursor: pointer; display:flex; align-items:center; justify-content:center;">✏️</button>
        </div>
        ...
        <div style="background: white; padding: 15px; border-radius: 16px; margin-bottom: 20px; border: 1px solid #f2f4f6;"><canvas id="priceChart" style="width:100%; height:150px;"></canvas></div>
        ${shopHtml}
        ${cashShopHtml}
        ${actionBtn}
        ${logoutBtn}
...
function renderRanking() {
    ...
    container.innerHTML = `
        <div class="info-card">
            ...
            ${top10.map((p, index) => {
                let rankIcon = index === 0 ? '🥇' : index === 1 ? '🥈' : index === 2 ? '🥉' : `<span style="color:#8b95a1; font-weight:bold; width:24px; text-align:center; display:inline-block;">${index+1}</span>`;
                ...
                return `
                    <div style="display: flex; justify-content: space-between; align-items: center; padding: 12px 10px; border-bottom: 1px solid #f9fafb; border-radius:8px; ${bg}">
                        <div style="display: flex; align-items: center; font-size: 15px; font-weight: bold;">
                            <span style="font-size: 20px; margin-right: 10px; display:inline-flex; align-items:center; justify-content:center;">${rankIcon}</span>
            ...
            if (pending) {
                actionBtn = `
                    <div style="display:flex; gap:8px; margin-top:12px;">
                        <button onclick="respondPending('${pending.id}', 'defend')" style="flex:1; padding:8px; background:#f39c12; color:white; border:none; border-radius:8px; font-weight:bold; cursor:pointer; font-size:12px;">⚖️ 이의제기 방어재판 발의 (1,000p)</button>
...
async function joinExistingRoom(codeParam = null) { 
    const code = codeParam || prompt("초대 코드를 입력하세요:"); if(!code || code.trim() === "") return; 
    showLoading();
    try { 
        ...
        const data = await res.json(); if(data.status === 'error') { if (!codeParam) alert(data.message); return; } 
...
async function leaveCurrentRoom() { 
    ...
    try { 
        ...
        const data = await res.json(); if(data.status === 'success') { showToast("클럽에서 퇴장했습니다."); currentRoomCode = null; await forceSync(); } else { alert(data.message); } 
    } catch(err) { alert("오류 발생"); } finally { hideLoading(); } 
}

async function sendChat() { 
    const input = document.getElementById('chat-input'); const text = input.value.trim(); if(!text || !currentRoomCode) return; input.value = ''; 
    try { 
...
function openFriendDetail(friendEmail) {
    ...
    modal.innerHTML = `
        <div style="background:white; padding:30px 25px; border-radius:20px; width:85%; max-width:340px; text-align:center; box-shadow: 0 10px 25px rgba(0,0,0,0.2); max-height:90vh; overflow-y:auto;">
            ...
            <div style="background:#f9fafb; padding:10px; border-radius:10px; font-size:12px; color:#8b95a1; margin-bottom:20px;">티켓은 1장 소모됩니다.<br>내 평가권: 👍 <b>${myProfile.goodTickets}장</b> | 👎 <b>${myProfile.badTickets}장</b></div>
            <div style="text-align:left; margin-bottom:15px;">
                <div style="font-weight:bold; margin-bottom:8px; font-size:13px; color:#4e5968;">1. 평가 종류 선택</div>
                <div style="display:flex; gap:10px;">
                    <button id="eval-type-good" onclick="selectEvalType('good')" style="flex:1; padding:12px; border:1px solid #ffdbdb; background:white; color:#ff3b30; border-radius:10px; font-weight:bold; cursor:pointer;">👍 호평하기</button>
                    <button id="eval-type-bad" onclick="selectEvalType('bad')" style="flex:1; padding:12px; border:1px solid #d6ebff; background:white; color:#3182f6; border-radius:10px; font-weight:bold; cursor:pointer;">👎 악평하기</button>
                </div>
            </div>
            <div id="eval-intensity-section" style="text-align:left; margin-bottom:15px; display:none;">
                ...
                <div style="display:flex; gap:8px;">
                    <button id="eval-int-1" onclick="selectEvalIntensity(1)" style="flex:1; padding:10px; border:1px solid #e5e8eb; background:white; border-radius:8px; font-weight:bold; cursor:pointer;"><span id="eval-int-1-pct">1%</span><br><span id="eval-int-1-pts" style="font-size:10px; font-weight:normal; color:#8b95a1;"></span></button>
                    <button id="eval-int-2" onclick="selectEvalIntensity(2)" style="flex:1; padding:10px; border:1px solid #e5e8eb; background:white; border-radius:8px; font-weight:bold; cursor:pointer;"><span id="eval-int-2-pct">2%</span><br><span id="eval-int-2-pts" style="font-size:10px; font-weight:normal; color:#8b95a1;"></span></button>
                    <button id="eval-int-3" onclick="selectEvalIntensity(3)" style="flex:1; padding:10px; border:1px solid #e5e8eb; background:white; border-radius:8px; font-weight:bold; cursor:pointer;"><span id="eval-int-3-pct">3%</span><br><span id="eval-int-3-pts" style="font-size:10px; font-weight:normal; color:#8b95a1;"></span></button>
                </div>
            </div>
            <div style="text-align:left; margin-bottom:20px;">
                <div style="font-weight:bold; margin-bottom:8px; font-size:13px; color:#4e5968;">3. 사유 작성</div>
                <textarea id="eval-reason-input" placeholder="평가 사유를 작성하세요 (필수)" style="width:100%; height:60px; padding:10px; border:1px solid #e5e8eb; border-radius:8px; box-sizing:border-box; resize:none; font-family:sans-serif; outline:none; font-size:13px;"></textarea>
            ...
            <button onclick="submitEvaluationFinal()" style="width:100%; padding:15px; background:#333d4b; color:white; border:none; border-radius:12px; font-weight:bold; font-size:15px; cursor:pointer; margin-bottom:10px;">🚀 평가 보내기</button>
            <button onclick="kickUser('${friend.email}', '${escapeHtml(friend.name)}')" style="width:100%; padding:12px; background:#f2f4f6; color:#4e5968; border:1px solid #e5e8eb; border-radius:12px; font-weight:bold; font-size:13px; margin-bottom:10px;">🚪 이 유저 내보내기 투표 발의</button>
            <button onclick="document.getElementById('eval-modal').style.display='none'" style="width:100%; padding:12px; background:#f2f4f6; color:#8b95a1; border:none; border-radius:12px; font-weight:bold; font-size:14px;">취소</button>
        </div>
    `;
...
window.kickUser = async function(targetEmail, targetName) {
    ...
    const reason = prompt(`[${targetName}] 님을 클럽에서 내보내는 투표를 시작합니다.\n내보내려는 사유를 적어주세요:`);
...
function selectEvalType(type) { 
    ...
    if (type === 'good') { 
        ...
        document.getElementById('anon-section').style.display = 'none'; document.getElementById('use-anon-ticket').checked = false;
    } else { 
    ...
    document.getElementById('eval-int-1-pts').textContent = `${type==='good'?'+':'-'}${evalState.p1.toLocaleString()}p`;
    document.getElementById('eval-int-2-pts').textContent = `${type==='good'?'+':'-'}${evalState.p2.toLocaleString()}p`;
    document.getElementById('eval-int-3-pts').textContent = `${type==='good'?'+':'-'}${evalState.p3.toLocaleString()}p`;
...
function submitEvaluationFinal() { if (!evalState.type) { alert("평가 종류를 선택해주세요."); return; } if (!evalState.intensity) { alert("변동폭을 선택해주세요."); return; } submitEvaluation(evalState.type, evalState.intensity); }
...
async function submitEvaluation(evalType, intensity) {
    ...
    if (!reasonText) { alert("사유를 반드시 작성해 주세요!"); return; }
    const anonCheckbox = document.getElementById('use-anon-ticket'); const isAnonymous = anonCheckbox && anonCheckbox.checked;
    ...
    try {
        ...
        const data = await res.json(); 
        if (data.status === 'success') { 
            if (evalType === 'good') triggerConfetti(); else triggerShake();
            alert(data.message); await forceSync(); 
        } else { alert(data.message); }
...
async function doDailyAttendance() { showLoading(); try { const res = await fetch(`${BACKEND_URL}/api/reward`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('fc_id_token')}` }, body: JSON.stringify({ email: myEmail, reward_type: 'attendance' }) }); const data = await res.json(); if(data.status === 'success') { myProfile = data.profile; showToast(data.message); saveData(); renderProfile(); triggerConfetti(); } else { showToast(data.message); } } catch(err) { alert("서버 오류"); } finally { hideLoading(); } }
...
function watchAd(type) {
    ...
    const btn = document.getElementById('ad-close-btn'); btn.disabled = true; btn.onclick = null;
    adInterval = setInterval(() => { timeLeft--; if (timeLeft > 0) { document.getElementById('ad-timer').textContent = `광고 시청 중... (${timeLeft}초)`; } else { clearInterval(adInterval); document.getElementById('ad-timer').textContent = "✅ 시청 완료!"; btn.disabled = false; btn.onclick = () => claimAdReward(false); } }, 1000);
}
...
async function claimWeeklyTickets() { showLoading(); try { const res = await fetch(`${BACKEND_URL}/api/reward`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('fc_id_token')}` }, body: JSON.stringify({ email: myEmail, reward_type: 'weekly' }) }); const data = await res.json(); if(data.status === 'success') { myProfile = data.profile; showToast(data.message); saveData(); renderProfile(); triggerConfetti(); } else { showToast(data.message); } } catch(err) { alert("서버 오류"); } finally { hideLoading(); } }
...
window.buyCashItem = async function(itemType) {
    ...
    if (itemType === 'megaphone') { 
        extraData = prompt("전국구 뉴스 티커에 띄울 메시지를 입력하세요 (최대 30자, 1시간 유지):"); if (!extraData || extraData.trim() === "") return; 
    }
    if (!confirm("💳 테스트 가상 결제가 진행됩니다. 계속하시겠습니까?")) return;
    showLoading();
    try {
        ...
        const data = await res.json(); 
        if (data.status === 'success') { showToast(data.message); await forceSync(); triggerConfetti(); } else { alert(data.message); }
    } catch(e) { alert("통신 오류"); } finally { hideLoading(); }
}
...
function openProfileModal() { 
    ...
    grid.innerHTML = DEFAULT_AVATARS.map(url => {
        ...
        const boxStyle = isSelected ? 'border: 3px solid #3182f6;' : 'border: 3px solid transparent;';
        return `<div onclick="selectDefaultProfile('${url}')" style="cursor: pointer; border-radius: 12px; overflow: hidden; background: #f2f4f6; ${boxStyle}"><img src="${url}" style="width: 100%; display: block;"></div>`;
    }).join(''); 

    const themeContainer = document.getElementById('owned-themes-container');
    if (themeContainer) {
        const owned = myProfile.ownedThemes || ['none'];
        const themes = [ { id: 'none', name: '일반 테마 (기본)', emoji: '👤' }, { id: 'neon', name: '홀로그램 네온', emoji: '✨', class: 'theme-neon' }, { id: 'fire', name: '지옥의 불꽃', emoji: '🔥', class: 'theme-fire' } ];
        themeContainer.innerHTML = themes.map(t => {
            if (!owned.includes(t.id)) return '';
            const isEquipped = myProfile.profileTheme === t.id;
            return `<div onclick="selectTheme('${t.id}')" class="${t.id !== 'none' ? t.class : ''}" style="cursor:pointer; padding:12px; border-radius:12px; background:${isEquipped ? '#e8f5e9' : '#f9fafb'}; display:flex; justify-content:space-between; align-items:center;"><b>${t.emoji} ${t.name}</b> ${isEquipped?'<span>장착 중</span>':''}</div>`;
        }).join('');
    }
}
...
window.selectTheme = function(themeId) { myProfile.profileTheme = themeId; saveData(); showToast("테마 장착 완료!"); openProfileModal(); renderProfile(); triggerConfetti(); };

async function changeNickname() { 
    const newName = prompt("변경할 닉네임 (최대 8자):"); if(!newName || newName.trim() === "" || newName.trim() === myProfile.name) return;
    try { 
        const res = await fetch(`${BACKEND_URL}/api/check-nickname?nickname=${encodeURIComponent(newName.trim())}`); const data = await res.json();
        if(!data.available) { alert(data.message); return; }
        myProfile.name = newName.trim(); myUsername = myProfile.name; localStorage.setItem('fc_username', myUsername); saveData(); showToast("변경 완료!"); renderProfile();
...
async function handleCredentialResponse(response) {
    ...
    try {
        ...
        if (loginIntent === 'login' && serverData.isNewUser) { alert("가입 정보가 없습니다."); localStorage.clear(); location.reload(); return; }
        myEmail = tempEmail;
...
async function submitNewNickname(googlePicture) {
    const inputEl = document.getElementById('new-nickname-input'); const errorEl = document.getElementById('nickname-error'); const newName = inputEl.value.trim(); if(!newName) return;
    try {
        const res = await fetch(`${BACKEND_URL}/api/check-nickname?nickname=${encodeURIComponent(newName)}`); const data = await res.json();
        ...
        myUsername = newName; localStorage.setItem('fc_username', myUsername); saveData();
...
async function initializeApp() { 
    try { 
        ...
        const overlay = document.getElementById('login-overlay'); if(overlay) overlay.remove(); finishSetup(); 
    } catch(err) { alert("서버 연결에 실패했습니다."); } finally { hideLoading(); }
}
...
function finishSetup() { 
    ...
    checkBadges(); updateTicker(); switchTab('home'); startAutoSync();
}
...
function drawPriceChart() {
    ...
    const history = myProfile.priceHistory; const labels = myProfile.timeHistory || [];
    ...
    myChartInstance = new Chart(ctx, { type: 'line', data: { labels: labels, datasets: [{ data: history, borderColor: lineColor, backgroundColor: bgColor, borderWidth: 3, pointRadius: 0, fill: true, tension: 0.4 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { display: true }, y: { display: true, position: 'right' } } } });
}
...
function drawFriendPriceChart(friend) {
    ...
    const history = friend.priceHistory || [friend.basePrice, friend.price];
    const isUp = history[history.length - 1] >= history[0]; const lineColor = isUp ? '#ff3b30' : '#3182f6'; const bgColor = isUp ? 'rgba(255, 59, 48, 0.1)' : 'rgba(49, 130, 246, 0.1)';
    friendChartInstance = new Chart(ctx, { type: 'line', data: { labels: history.map(() => ''), datasets: [{ data: history, borderColor: lineColor, backgroundColor: bgColor, borderWidth: 3, pointRadius: 0, fill: true, tension: 0.4 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { display: false }, y: { display: true, position: 'right' } } } });
}